...
router.get('/account', (req, res) => {
  // usename; email; gre_q; gre_v; gre_awa; gpa; status; dream_area 
  // Assuming username is provided via query string
  // const user_id = req.query.user_id;
  const user_id = '00023e5e59'
  // Replace 'table_name' with your actual user info table name
  ...
  connection.query(sql, [user_id], function(err, result) {
    ...
    if(result.length === 0) {
      ...
      res.send({ message: 'User info retrieved successfully', userInfo: result[0] });
    }
  });
});
